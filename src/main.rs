...
use systems::{
    camera::init_camera,
    terminal::{
        //init_terminal, render_terminal, update_sidebars, GameLog,
        init_terminal,
        Terminal,
    },
};
...
// TODO: you can use substates, this may be useful during the game
pub enum AppState {
    MainMenu,
    #[default]
    NewGame,
...
fn main() {
    ...
    // let (screen_width, screen_height) = (1080., 720.);

    // App Builder.
    App::new()
        // DefaultPlugin should be first thing to add, other things depend on it
        .add_plugins((
            DefaultPlugins
                .set(WindowPlugin {
                    primary_window: Some(Window {
                        title: "Space Trader".into(),
                        name: Some("space_trader.app".into()),
                        ...
                        visible: true, // False will result in no window
                        ..default()
                    }),
                    ..default()
                })
                .set(ImagePlugin::default_nearest()), // prevents blurring
            LogDiagnosticsPlugin::default(),
            // FrameTimeDiagnosticsPlugin::default(),
        ))
        .init_state::<AppState>()
        .insert_resource(ClearColor(Color::BLACK)) // Sets default bg color
        .insert_resource(terminal)
        .add_systems(Startup, init_terminal)
